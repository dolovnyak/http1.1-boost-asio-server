...
/// 400
BadRequest::BadRequest(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::BadRequest, "Bad request", false, server_config) {}

BadHttpVersion::BadHttpVersion(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::BadRequest, "Bad HTTP version", false, server_config) {}

BadFirstLine::BadFirstLine(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::BadRequest, "Bad first line", false, server_config) {}

BadHeader::BadHeader(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::BadRequest, "Bad header", false, server_config) {}

BadContentLength::BadContentLength(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::BadRequest, "Bad content length", false, server_config) {}

BadChunkSize::BadChunkSize(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::BadRequest, "Bad chunk size", false, server_config) {}

BadChunkBody::BadChunkBody(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::BadRequest, "Bad chunk body", false, server_config) {}


/// 404
NotFound::NotFound(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::NotFound, "Not found", true, server_config) {}


/// 405
MethodNotAllowed::MethodNotAllowed(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
        : HttpException(message, Http::MethodNotAllowed, "Method not allowed", true, server_config) {}


/// 411
LengthRequired::LengthRequired(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
...
/// 501
NotImplemented::NotImplemented(const std::string& message,  const Optional<SharedPtr<ServerConfig> >& server_config)
