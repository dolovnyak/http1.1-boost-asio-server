    ...
    for (auto& raw_method: raw_methods) {
        ...
        switch (method) {
            ...
            case Http::Method::Patch:
                throw std::runtime_error("Method " + raw_method + " not implemented");
...
HttpReturn tag_invoke(const boost::json::value_to_tag<HttpReturn>&, boost::json::value const& json) {
    ...
    std::optional<std::string> redirect = exceptions_wrapper(extract_soft<std::optional<std::string>>, "Redirect", obj,
                                                             std::nullopt);
...
tag_invoke(const boost::json::value_to_tag<std::shared_ptr<Location>>&, boost::json::value const& json) {
    ...
    std::string location = exceptions_wrapper(extract<std::string>, "Location", obj);
...
std::shared_ptr<ExtensionInterceptor>
tag_invoke(const boost::json::value_to_tag<std::shared_ptr<ExtensionInterceptor>>&, const boost::json::value& json) {
    const boost::json::object& obj = json.as_object();

    return std::make_shared<ExtensionInterceptor>(
            exceptions_wrapper(extract<std::string>, "Extension", obj),
            exceptions_wrapper(extract<std::string>, "Cgi-path", obj),
            ToHandledMethods(exceptions_wrapper(sequence_container_extract_soft<std::vector<std::string>>,
                                                "OnMethods", obj, std::vector<std::string>())));
}
...
tag_invoke(const boost::json::value_to_tag<std::shared_ptr<ServerConfig>>&, const boost::json::value& json) {
    ...
    return std::make_shared<ServerConfig>(
            ...
            exceptions_wrapper(
                    sequence_container_extract_soft<std::vector<std::shared_ptr<ExtensionInterceptor>>>,
                    "ExtensionInterceptors", obj, std::vector<std::shared_ptr<ExtensionInterceptor>>()),
