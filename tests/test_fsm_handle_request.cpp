...
TEST(Request, Check_Init) {
    ...
    ASSERT_EQ(request._parse_state, RequestParseState::HandleFirstLine);
    ASSERT_EQ(request.body, "");
    ASSERT_EQ(request.raw_request, "");
    ASSERT_EQ(request.http_headers.size(), 0);
    ASSERT_EQ(request._parsed_size, 0);
...
TEST(Request, Handle_FSM_First_Line) {
...

    {
        ...
        EXPECT_NO_THROW(res = request.Parse(MakeShared(first_line)));
        ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
        ASSERT_EQ(request._parse_state, RequestParseState::HandleHeader);
        ASSERT_EQ(request._parsed_size, total_size);
        ASSERT_EQ(request.raw_request, full_raw_request);
    ...
    EXPECT_NO_THROW(res = request.Parse(MakeShared(first_line_part1)));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleFirstLine);
    ASSERT_EQ(request._parsed_size, 6); /// empty lines
    ASSERT_EQ(request.raw_request, full_raw_request);
    ASSERT_EQ(request.raw_method, "");
    ASSERT_EQ(request.target.full_path, "");
    ASSERT_EQ(request.raw_http_version.major, 0);
    ...
    EXPECT_NO_THROW(res = request.Parse(MakeShared(first_line_part2)));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleHeader);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request);
...
TEST(Request, Handle_FSM_Headers) {
    ...
    res = request.Parse(MakeShared(first_line));

    /// _http_headers
    const std::string host_header = "hoST: 123\r\n";
    total_size += host_header.size();
    full_raw_request += host_header;
    EXPECT_NO_THROW(res = request.Parse(MakeShared((host_header))));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleHeader);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request);
    ASSERT_EQ(request.http_headers.size(), 1);
    ASSERT_EQ(request.http_headers["host"][0], "123");

    const std::string check1_header = "check1:     1  \t  2  3    \r\n";
    total_size += check1_header.size();
    full_raw_request += check1_header;
    EXPECT_NO_THROW(res = request.Parse(MakeShared((check1_header))));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleHeader);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request);
    ASSERT_EQ(request.http_headers.size(), 2);
    ASSERT_EQ(request.http_headers["check1"][0], "1  \t  2  3"); /// because of strip

    const std::string check2_header_part1 = "check2: \t first part, ";
    full_raw_request += check2_header_part1;
    EXPECT_NO_THROW(res = request.Parse(MakeShared((check2_header_part1))));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleHeader);
    ASSERT_EQ(request._parsed_size, total_size); /// handled_size is not changed
    ASSERT_EQ(request.raw_request, full_raw_request);
    ASSERT_EQ(request.http_headers.size(), 2);
    ...
    EXPECT_NO_THROW(res = request.Parse(MakeShared((check2_header_part2))));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleHeader);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request);
    ASSERT_EQ(request.http_headers.size(), 3);
    ASSERT_EQ(request.http_headers["check2"][0], "first part, second part");

    const std::string crln = "\r\n";
    full_raw_request += crln;
    total_size += crln.size();
    EXPECT_NO_THROW(res = request.Parse(MakeShared((crln))));
    ASSERT_EQ(res, RequestParseStatus::Finish);
    ASSERT_EQ(request._parse_state, RequestParseState::FinishHandle);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request);
}
...
TEST(Request, Handle_FSM_Body_By_Content_Length) {
    ...
    EXPECT_NO_THROW(res = request.Parse(MakeShared(content_length_header)));
    ASSERT_EQ(request.http_headers[CONTENT_LENGTH][0], "10");

    const std::string host_header = "host: a\r\n";
    total_size += host_header.size();
    full_raw_request += host_header;
    EXPECT_NO_THROW(res = request.Parse(MakeShared(host_header)));
    ASSERT_EQ(request.http_headers[HOST][0], "a");

    total_size += CRLF_LEN;
    full_raw_request += CRLF;
    EXPECT_NO_THROW(res = request.Parse(MakeShared(std::string(CRLF))));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleBodyByContentLength);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request);
    ...
    EXPECT_NO_THROW(res = request.Parse(MakeShared(body + "a")));
    ASSERT_EQ(res, RequestParseStatus::Finish);
    ASSERT_EQ(request._parse_state, RequestParseState::FinishHandle);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request + "a"); // handled_size different with full_raw_request size
...
TEST(Request, Handle_FSM_Chunked_Body) {
    ...
    EXPECT_NO_THROW(res = request.Parse(MakeShared(transfer_encoding_header)));
    ASSERT_EQ(request.http_headers[TRANSFER_ENCODING][0], "chunked");
    ASSERT_EQ(request._parse_state, RequestParseState::HandleChunkSize);

    const std::string chunk1 = "5\r\n01234\r\n";
    total_size += chunk1.size();
    full_raw_request += chunk1;
    full_body += "01234";
    EXPECT_NO_THROW(res = request.Parse(MakeShared(chunk1)));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleChunkSize);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request);
    ...
    EXPECT_NO_THROW(res = request.Parse(MakeShared(chunk2_size)));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ASSERT_EQ(request._parse_state, RequestParseState::HandleChunkBody);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request);
    ...
    EXPECT_NO_THROW(res = check_incorrect_chunk_request.Parse(MakeShared(chunk2_body + "f")));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    EXPECT_ANY_THROW(res = check_incorrect_chunk_request.Parse(MakeShared(std::string(CRLF))));
    /// end check error

    EXPECT_NO_THROW(res = request.Parse(MakeShared(chunk2_body)));
    ASSERT_EQ(res, RequestParseStatus::WaitMoreData);
    ...
    EXPECT_NO_THROW(res = request.Parse(MakeShared(chunk_end + "a")));
    ASSERT_EQ(res, RequestParseStatus::Finish);
    ASSERT_EQ(request._parse_state, RequestParseState::FinishHandle);
    ASSERT_EQ(request._parsed_size, total_size);
    ASSERT_EQ(request.raw_request, full_raw_request + "a"); /// for now raw_request could contain spam data in the end
    ASSERT_EQ(request.body, full_body);
